...
const PreparationStepList = ({
  ...
  handleAddPreparationStepRow,
  handleRemovePreparationStepRow,
}) => {
  return (
    <section style={{ marginTop: '25px' }}>
      <h2>
        Instructions <span>(enter instructions, each step at a time)</span>
      </h2>
          ...
          <Grid container spacing={2} key={index}>
            <Grid item xs={11}>
              <TextField
                ...
                value={step}
                onChange={event =>
                  handlePreparationStepInputChange(event, 'instruction', index)
                }
            ...
            {index !== 0 ? (
              <Grid item xs={1}>
                <button
                  onClick={event =>
                    handleRemovePreparationStepRow(event, index)
                  }
                  className="remove"
